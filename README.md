...
- Conflicts: `]x` and `[x` next and previous conflict, `<leader>cm` open a picker menu for chunk resolution, `<leader>co` choose ours, `<leader>ct` choose theirs, `<leader>cb` choose both, `<leader>c0` choose none, `<leader>cq` list conflicts in quickfix, `<leader>cv` open merge diff view, `<leader>cV` close merge diff view. Diffview merge mode defaults to one editable `LOCAL` pane with conflict markers (`<<<<<<<`, `|||||||`, `>>>>>>>`); press `g<C-x>` inside Diffview to cycle to multi-pane layouts when needed. Conflict sections use green (incoming) and red (current) backgrounds.
